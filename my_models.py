...
# ALL MODELS SHOULD TAKE RAW BOARD. SPLITTING SHOULD BE DONE INTERNALLY
# get it training -> main difference with working_mofo is split board and gradient!
...
class split_board_model():
    ...
    def __init__(self,nrows,ncols,nactions,nfilters=8,f_size=4,h_size=10,d_rate=0.01,l_rate=0.01):
        ...
        if nfilters>0:
            ...
            # for now let's ignore 2d+3d info from input and reshape the board into a 1D array
            # also remove the empy third layer as this just complicates things
            input_state_sep01,input_state_sep2 = tf.split(self.input_state_sep,[2,1],axis=3)
            self.input_flat = tf.reshape(input_state_sep01, [-1, nrows * ncols * 2],name="input_flat_layer")
